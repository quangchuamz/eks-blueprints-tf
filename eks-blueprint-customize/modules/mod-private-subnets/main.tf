...
resource "aws_route_table" "private_rtb" {
  count = local.create_private_subnets   ? var.nat_gw_count : 0
...
#  }
  ...
  tags = merge(
    {
      "Name" = var.nat_gw_count == 1 ? "${var.vpc_name}-${var.private_subnet_suffix}" : format(
...
resource "aws_route_table_association" "private_rtb_association" {
  ...
  route_table_id = element(
    ...
    var.nat_gw_count == 1 ? 0 : count.index,
  )
}
#quangchu
#resource "aws_route" "priave_dst_route" {
